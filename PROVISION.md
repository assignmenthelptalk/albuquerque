...
- Part 2: `Water Softeners of [City]` (brand — constant across every page)
...
Full pattern:
```
[Service] in [City], [State] | Water Softeners of [City]: Water Softener Installation and Repair
```
...
appends `| Water Softeners of {site.city}: Water Softener Installation and
...
| About | `About Water Softeners of [City]` |
| Contact | `Contact Water Softeners of [City]` |
...
inner-page pattern:
```
Water Softeners of [City] - Premier Water Softener Installation in [City], [State]
```
...
| About | `About Water Softeners of [City] \| Trusted Local Specialists` |
| Contact | `Contact Water Softeners of [City] \| Trusted Local Specialists` |
...
Every page opens with a lead paragraph directly below the page header,
styled with the `.page-opening` class (`global.css`). The pattern differs
by page type:

**Homepage — "Brand IS" pattern.** No backlink (the brand text isn't a
link, since it's already on the page it would link to). States what the
business is, what it does, and folds in the local GPG/water-authority
data point:
```
Water Softeners of [City] is a professional water softener installation
and service company providing salt-based softener installation, repair,
resin bed replacement, brine tank cleaning, and water quality testing
across [City] and [County]. [Water Authority] delivers [GpgLow]–[GpgHigh]
GPG water to every [City] home, classified as [GpgLabel] by the Water
Quality Association. Every system we install is sized to your confirmed
GPG reading and household water usage, not a statewide estimate. Free
in-home water test available. No obligation.
```

**Inner pages — "Brand OFFERS/provides" pattern.** Opens with the brand
name as a hyperlink to `/` (class `brand-link`), then "offers" or
"provides" (never "is a" — inner pages describe what the business does
...
GPG/water-authority data point:
```
<a href="/" class="brand-link">Water Softeners of [City]</a> offers
[service] across [City] and [County]. [Page-specific detail, often
including [GpgLow]–[GpgHigh] GPG and/or [Water Authority]].
```

Rules:
1. Homepage uses "is a" — every inner page uses "offers" or "provides,"
   never "is a."
2. Inner-page opening brand mention is always a hyperlink to `/` via
   `.brand-link`; homepage's is plain text (not a link to itself).
3. No em dashes anywhere in any opening paragraph — use separate
   sentences, commas, or colons instead. (The en dash in GPG ranges,
   e.g. `11–15`, is unrelated and stays as-is throughout the codebase.)
...
CSS: `.page-opening` (body copy sizing/color/line-height for the
paragraph) and `.brand-link` (`font-weight: 700`, inline text link
style for the inner-page brand mention) — both in `global.css`, themed
via the existing color custom properties so the pattern carries over
unchanged to any city's color scheme.
